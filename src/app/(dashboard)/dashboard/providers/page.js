...
export default function ProvidersPage() {
  ...
  // Helper to determine priority tier for API Key providers:
  // Tier 1: Có key AND đang active (has key, active)
  // Tier 2: Có key AND không active (has key, inactive/disabled)
  // Tier 3: Không key, không active (no key)
  const getApikeyTier = (key) => {
    const stats = getProviderStats(key, "apikey");
    const hasKey = stats.total > 0;
    const isActive = hasKey && !stats.allDisabled;
    if (hasKey && isActive) return 1;
    if (hasKey && !isActive) return 2;
    return 3;
  };

  // API Key: ưu tiên có key & active > có key & inactive > không key, không active
  const apikeyEntries = Object.entries(APIKEY_PROVIDERS)
    ...
    .sort(([ka, a], [kb, b]) => {
      const tierA = getApikeyTier(ka);
      const tierB = getApikeyTier(kb);
  ...
  // Collapse/expand: luôn ưu tiên hiển thị tất cả các provider có key và đang active
  const activeKeyCount = apikeyEntries.filter(([k]) => getApikeyTier(k) === 1).length;
  const initialLimit = Math.max(activeKeyCount, APIKEY_INITIAL_VISIBLE);
  const visibleApikeyEntries =
    isApikeySearching || showAllApikey
      ? apikeyEntries
      : apikeyEntries.slice(0, initialLimit);
  const hiddenApikeyCount = Math.max(0, apikeyEntries.length - initialLimit);
        ...
        {!isApikeySearching && hiddenApikeyCount > 0 && (
          ...
          >
            <span className="material-symbols-outlined text-[16px]">
              {showAllApikey ? "expand_less" : "expand_more"}
            </span>
            {showAllApikey
