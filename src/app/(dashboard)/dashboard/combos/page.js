...
function ComboCard({
  ...
  const rawModelNames = combo.models.map(getModelName);
  const comboCaps = aggregateComboCapabilities(rawModelNames, comboByName);

  return (
    <Card padding="sm" className={`group ${selected ? "ring-1 ring-primary/40 bg-primary/[0.03]" : ""} ${!isActive ? "opacity-60" : ""}`}>
                ...
                combo.models.map((model, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <CapacityBadges caps={
                        comboByName[name]
                          ? aggregateComboCapabilities(comboByName[name], comboByName)
                          : getCaps?.(name)
                      } />
                    </button>
                  );
