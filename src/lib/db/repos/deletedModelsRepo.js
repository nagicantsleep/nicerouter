...
export async function getDeletedByProvider(providerAlias) {
...
export async function deleteModels(providerAlias, ids) {
...
export async function restoreDeletedModels(providerAlias, ids) {
  if (!providerAlias) return;
  const db = await getAdapter();
  db.transaction(() => {
