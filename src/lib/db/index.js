...
export async function exportDb() {
  ...
  const out = {
    ...
    combos: db.all(`SELECT * FROM combos`).map((r) => ({ id: r.id, name: r.name, kind: r.kind, models: parseJson(r.models, []), isActive: r.isActive === undefined || r.isActive === null ? true : r.isActive === 1, createdAt: r.createdAt, updatedAt: r.updatedAt })),
    modelAliases: {},
    customModels: [],
    mitmAlias: {},
    pricing: {},
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    for (const c of payload.combos || []) {
      db.run(
        `INSERT OR REPLACE INTO combos(id, name, kind, models, isActive, createdAt, updatedAt) VALUES(?, ?, ?, ?, ?, ?, ?)`,
        [c.id, c.name, c.kind || null, stringifyJson(c.models || []), c.isActive === false ? 0 : 1, c.createdAt || new Date().toISOString(), c.updatedAt || new Date().toISOString()]
      );
    }
